...
public class NetworkDispatcher extends Thread {
        ...
        while (true) {
            ...
            try {
                ...
                try {
                    ...
                    VolleyLog.m1688e(e, "Unhandled exception %s", e.toString());
    ...
    private void showCache(Request<?> request) {
        if (request.shouldCache()) {
            Entry entry = this.mCache.get(request.getCacheKey());
            if (entry != null) {
                this.mDelivery.postResponse(request, request.parseNetworkResponse(new NetworkResponse(entry.data, entry.responseHeaders)));
            }
        }
    }
}
