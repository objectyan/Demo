...
public abstract class AsyncHttpResponseHandler implements ResponseHandlerInterface {
    ...
    public C6327f[] getRequestHeaders() {
        return this.requestHeaders;
    }

    public void setRequestHeaders(C6327f[] requestHeaders) {
    ...
    public void setUseSynchronousMode(boolean sync) {
        if (!sync && this.looper == null) {
            sync = true;
            AsyncHttpClient.log.mo4891w(LOG_TAG, "Current thread has not called Looper.prepare(). Forcing synchronous mode.");
        }
    ...
    public void onProgress(long bytesWritten, long totalSize) {
        ...
        logInterface.mo4889v(str, String.format(str2, objArr));
    }
    ...
    public void onRetry(int retryNo) {
        AsyncHttpClient.log.mo4878d(LOG_TAG, String.format("Request retry no. %d", new Object[]{Integer.valueOf(retryNo)}));
    }

    public void onCancel() {
        AsyncHttpClient.log.mo4878d(LOG_TAG, "Request got cancelled");
    }

    public void onUserException(Throwable error) {
        AsyncHttpClient.log.mo4881e(LOG_TAG, "User-space exception detected!", error);
    ...
    public final void sendSuccessMessage(int statusCode, C6327f[] headers, byte[] responseBytes) {
        sendMessage(obtainMessage(0, new Object[]{Integer.valueOf(statusCode), headers, responseBytes}));
    }

    public final void sendFailureMessage(int statusCode, C6327f[] headers, byte[] responseBody, Throwable throwable) {
            ...
            switch (message.what) {
                case 0:
                    ...
                    if (response == null || response.length < 3) {
                        AsyncHttpClient.log.mo4880e(LOG_TAG, "SUCCESS_MESSAGE didn't got enough params");
                        return;
                    } else {
                        onSuccess(((Integer) response[0]).intValue(), (C6327f[]) response[1], (byte[]) response[2]);
                        return;
                    }
                case 1:
                    response = (Object[]) message.obj;
                    if (response == null || response.length < 4) {
                        AsyncHttpClient.log.mo4880e(LOG_TAG, "FAILURE_MESSAGE didn't got enough params");
                        return;
                    } else {
                        onFailure(((Integer) response[0]).intValue(), (C6327f[]) response[1], (byte[]) response[2], (Throwable) response[3]);
                        return;
                    }
                case 2:
                    onStart();
                ...
                case 4:
                    ...
                    if (response == null || response.length < 2) {
                        AsyncHttpClient.log.mo4880e(LOG_TAG, "PROGRESS_MESSAGE didn't got enough params");
                        return;
                    } else {
                ...
                case 5:
                    ...
                    if (response == null || response.length != 1) {
                        AsyncHttpClient.log.mo4880e(LOG_TAG, "RETRY_MESSAGE didn't get enough params");
                        return;
                    } else {
    ...
    public void sendResponseMessage(C6228x response) throws IOException {
        if (!Thread.currentThread().isInterrupted()) {
            ...
            byte[] responseBody = getResponseData(response.mo5075b());
            if (!Thread.currentThread().isInterrupted()) {
                if (status.mo5266b() >= 300) {
                    sendFailureMessage(status.mo5266b(), response.getAllHeaders(), responseBody, new C6266l(status.mo5266b(), status.mo5267c()));
                } else {
                    sendSuccessMessage(status.mo5266b(), response.getAllHeaders(), responseBody);
                }
            }
        }
    }
    ...
    byte[] getResponseData(C3008n entity) throws IOException {
        ...
        if (entity != null) {
            ...
            if (instream != null) {
                ...
                try {
                    C6583c buffer = new C6583c(contentLength <= 0 ? 4096 : (int) contentLength);
