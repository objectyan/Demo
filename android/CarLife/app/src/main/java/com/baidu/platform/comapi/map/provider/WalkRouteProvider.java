...
public class WalkRouteProvider implements RenderProvider {
    ...
    public WalkRouteProvider(WalkPlan walkRoute, boolean indoor) {
        this.mWalkRoute = walkRoute;
        this.mHasIndoor = indoor;
    ...
    public String getProjectionPaths() {
        ...
        Routes routes = this.mWalkRoute.getRoutes(0);
        if (routes == null) {
            return null;
        }
        for (int j = 0; j < routes.getLegsCount(); j++) {
            for (int i = 0; i < routes.getLegs(j).getStepsCount(); i++) {
                jsonBuilder.object();
                jsonBuilder.key("path").objectValue(ProviderUtils.pathToJson(routes.getLegs(j).getSteps(i).getSpathList()));
    ...
    private void addRouteEndNode() {
        if (this.mWalkRoute.getOption().getEndCount() >= 1) {
            End endNode = this.mWalkRoute.getOption().getEnd(this.mWalkRoute.getOption().getEndCount() - 1);
            if (endNode != null) {
                this.mJsonBuilder.object();
                this.mJsonBuilder.key("ud").value(endNode.hasUid() ? endNode.getUid() : "");
                this.mJsonBuilder.key("ty").value(2);
                this.mJsonBuilder.key("nst").value(357);
                this.mJsonBuilder.key("fst").value(357);
                this.mJsonBuilder.key("tx").value(endNode.hasWd() ? endNode.getWd() : "终点");
                this.mJsonBuilder.key("in").value(getCurrentIndex());
                this.mJsonBuilder.key(OVERLAY_KEY.ALIGN).value(2);
                this.mJsonBuilder.key("path").objectValue(ProviderUtils.pathToJson(endNode.getSptList()));
                this.mJsonBuilder.endObject();
            }
        }
    }

    private void addRouteLegData(Legs legs) {
        if (this.mWalkRoute.getOption().getEndCount() >= 1) {
    ...
    private void resetTemporaryField() {
        ...
        if (this.mWalkRoute != null && this.mWalkRoute.getRoutesCount() > 0 && this.mWalkRoute.getRoutes(0).getLegsCount() > 0 && this.mWalkRoute.getRoutes(0).getLegs(0).hasLegLinked() && this.mWalkRoute.getRoutes(0).getLegs(0).getLegLinked().hasPrev()) {
            int prev = this.mWalkRoute.getRoutes(0).getLegs(0).getLegLinked().getPrev();
            if (prev < this.mWalkRoute.getIndoorNavisCount()) {
                IndoorNavi indoorNavi = this.mWalkRoute.getIndoorNavis(prev);
                if (indoorNavi.getRoutesCount() > 0 && indoorNavi.getRoutes(0).getLegsCount() > 0) {
                    this.mNodeCount = indoorNavi.getRoutes(0).getLegs(0).getStepsCount();
                }
            }
        }
    }
    ...
    private void addStartDashRoutes() {
        ...
        if (startPt != null) {
            ...
            if (x != 0 && y != 0) {
                ...
                if (firstRoutePoint != null && this.mSpathType == 0) {
                    firstRoutePoint = CoordinateUtil.gcj02Tobd09mc((double) (((float) firstRoutePoint.getIntX()) / 100000.0f), (double) (((float) firstRoutePoint.getIntY()) / 100000.0f));
                }
