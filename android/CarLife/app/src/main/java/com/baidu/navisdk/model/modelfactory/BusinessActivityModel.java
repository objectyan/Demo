...
public class BusinessActivityModel {
    ...
    public long parkingStartTime = -1;
        ...
        public String toString() {
            ...
            if (this.list != null) {
                for (int i = 0; i < this.list.length; i++) {
                    listStr = listStr + this.list[i];
                    if (i < this.list.length - 1) {
                        listStr = listStr + ", ";
                    }
                }
            }
            return "cardType: " + this.cardType + ", hint: " + this.hint + ", unlock: " + this.unlock + ", tip: " + this.tip + ", hlink: " + this.hlink + ", hicon: " + this.hicon + ", list: " + listStr;
        }
    }

    public void resetTrafficJam() {
        this.isTrafficJam = false;
        LogUtil.m15791e(BusinessActivityManager.TAG, "resetTrafficJam() ");
    }

    public void resetParking() {
        this.isParking = false;
        this.parkingStartTime = -1;
        LogUtil.m15791e(BusinessActivityManager.TAG, "resetParking() ");
    }

    public boolean isNeedUpload() {
        if (1 == this.naviendNeedUpload) {
            return true;
        }
        if (2 == this.naviendNeedUpload && this.isPrizeReceived) {
            return true;
        }
        return false;
    }
    ...
    public boolean isUploadDataContainsValidBduss() {
        if (this.uploadBundleData == null || !this.uploadBundleData.containsKey("pcBduss") || TextUtils.isEmpty(this.uploadBundleData.getString("pcBduss"))) {
            return false;
        }
        return true;
    }
    ...
    public boolean isUploadDataContainsValidBdussoForNaving() {
        if (this.uploadBundleDataForNaving == null || !this.uploadBundleDataForNaving.containsKey("pcBduss") || TextUtils.isEmpty(this.uploadBundleDataForNaving.getString("pcBduss"))) {
            return false;
        }
        return true;
    }
    ...
    private void saveBitmap(Bitmap bitmap, String absPath) {
        if (bitmap != null && !TextUtils.isEmpty(absPath)) {
