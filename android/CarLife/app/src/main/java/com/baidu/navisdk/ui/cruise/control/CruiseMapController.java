...
public class CruiseMapController {
    ...
    private BNMapObserver mMapObserver = new C42742();

    /* renamed from: com.baidu.navisdk.ui.cruise.control.CruiseMapController$2 */
    class C42742 implements BNMapObserver {
        C42742() {
        }
    ...
    private void setMapInitScaleLevel() {
        ...
        int level = readMapScaleLevel();
        mapStatus._Level = (float) level;
        mapStatus._Rotation = 0;
        LogUtil.m15791e("Cruise", "init map level " + level);
    ...
    public void setMapStatus(LocData locData, boolean isNorth2D, boolean withAnim) {
        ...
        if (mapStatus != null) {
            ...
            mapStatus._Xoffset = getMapXOffset(false);
            mapStatus._Yoffset = getMapYOffset(false, isNorth2D);
    ...
    private long getMapYOffset(boolean isPortrait, boolean isNorth2D) {
        long offset;
        if (isNorth2D) {
            if (isPortrait) {
                offset = (long) ((-ScreenUtil.getInstance().dip2px(70)) / 2);
            }
            offset = (long) ScreenUtil.getInstance().dip2px(40);
        } else {
            long height;
            if (isPortrait) {
                height = (long) (ScreenUtil.getInstance().getHeightPixels() - ScreenUtil.getInstance().dip2px(70));
            } else {
                height = (long) ScreenUtil.getInstance().getWidthPixels();
            }
            offset = ((-height) / 4) + ((long) ScreenUtil.getInstance().dip2px(40));
        }
        LogUtil.m15791e("Cruise", "getMapYOffset: isPortrait " + isPortrait + ", is2DNorth " + isNorth2D + ", Y offset " + offset);
        return offset;
    }
